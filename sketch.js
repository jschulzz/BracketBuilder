...
function setup() {
  ...
  team(
    ...
    width / 2 + 50,
    height / 2 + 100,
    textWidth(r2[1].team) + 50,
    1.5
  );
  team(r1[0], width / 2, height / 2, textWidth(r1[0].team) + 50, 2.2);

  // makeSide(r2, 750)
  textSize(20);
  text(data.method, width / 2, 700);
  noLoop();
  saveCanvas(c, data.method + "__" + getDay(), "png");
}
function draw() {}
...
function makeSide(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (textWidth(arr[i].team) > maxW) {
      maxW = textWidth(arr[i].team);
...
function team(info, centerx, centery, w, s) {
  ...
  if (info.matchup_chance < 0.53) {
    ...
    fill(255, map(info.matchup_chance, .53, .65, 200, 255), 200);
  }
  rectMode(RADIUS);
