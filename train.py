...
def createInputList(t1, t2):
    ...
    output_current.append(sum(t1["player_pts"][:5])/100)  # average jersey number
    output_current.append(sum(t1["player_pts"][5:])/t1["pts_per_g"])  # average jersey number
    output_current.append(sum(t1["player_pts"][:5])/t1["pts_per_g"])  # average jersey number
    ...
    output_current.append(sum(t2["player_pts"][:5])/100)  # average jersey number
    output_current.append(sum(t2["player_pts"][5:])/t2["pts_per_g"])  # average jersey number
    output_current.append(sum(t2["player_pts"][:5])/t2["pts_per_g"])  # average jersey number
    # average height of starters
...
def createModel(input_length):
    ...
    model.add(Dense(20, activation='relu', kernel_initializer="normal", input_shape=(input_length,)))
...
if __name__ == "__main__":
    ...
    reduction = ReduceLROnPlateau(monitor='val_loss', factor=0.8, patience=10,
                                mode='auto', cooldown=2, min_lr=0.00001)
    ...
    print("Fitting Now")
    model.fit(np.asarray(input_list), np.asarray(output_list), epochs=100,
            batch_size=8, callbacks=callbacks_list, validation_split=0.2)


# estimators = []
# estimators.append(('standardize', StandardScaler()))
# estimators.append(('mlp', KerasClassifier(model, epochs=100, batch_size=5, verbose=0)))
# pipeline = Pipeline(estimators)
# kfold = StratifiedKFold(n_splits=10, shuffle=True)
# results = cross_val_score(pipeline, output_list, input_list, cv=kfold)
