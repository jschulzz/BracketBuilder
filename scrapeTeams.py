...
with open("games.json", 'r+') as f:
    data = json.load(f)
    for team in data:
        print(str(list(data.keys()).index(team)) +
              "/" + str(len(list(data.keys()))), team)
        ...
        soup.prettify()
        heights = list(map(lambda x: int(float(x.get("csk", -1))), soup.find_all(
            "td", attrs={"data-stat": "height"})))
        # print(heights)
        heights = list(filter(lambda x: x > 0, heights))
        weights = list(map(lambda x: int(x.get("csk", -1)), soup.find_all(
            "td", attrs={"data-stat": "weight"})))
        weights = list(filter(lambda x: x > 0, weights))
        player_pts = list(map(lambda x: str(x.get("csk", "_")), soup.find_all(
            "td", attrs={"data-stat": "summary"})))
        player_pts = list(filter(lambda x: x != "_" and x[:2] != "0-", player_pts))
        # print(player_pts)
        player_pts = list(map(lambda x: int(x.split("-")[1])/int(x.split("-")[0]), player_pts))
        nums = list(map(lambda x: int(x.string) if x.string != None else -1, soup.find_all(
            "td", attrs={"data-stat": "number"})))
        nums = list(filter(lambda x: x != -1, nums))
        hometowns = list(map(lambda x: (x.string) if x.string != None else -1, soup.find_all(
            "td", attrs={"data-stat": "hometown"})))
        hometowns = list(filter(lambda x: x != -1, hometowns))
        # print(hometowns)
        # O_rtg = soup.find(text=re.compile("ORtg:"))
        O_rtg = float(soup.find(text=re.compile("ORtg:")).parent.parent.getText().split(" ")[1])
        D_rtg = float(soup.find(text=re.compile("DRtg:")).parent.parent.getText().split(" ")[1])
        SOS = float(soup.find("a", text=re.compile("SOS")).parent.parent.getText().split(" ")[1])
        PS = float(soup.find(text=re.compile("PS/G:")).parent.parent.getText().split(" ")[1])
        PA = float(soup.find(text=re.compile("PA/G:")).parent.parent.getText().split(" ")[1])
        team_stats_row = soup.find("table", attrs={"id":"team_stats"}).find_all("tr")[1]
